...
class UserEventRepository:
    ...
    def list_unlinked_for_session(self, dealership_id: str, session_id: str):
        return (
            self.db.query(UserEvent)
            .filter(
                UserEvent.dealership_id == dealership_id,
                UserEvent.session_id == session_id,
                UserEvent.lead_public_id.is_(None),
            )
            .order_by(UserEvent.id.asc())
            .all()
        )

    def link_session_events_to_lead(self, dealership_id: str, session_id: str, lead_public_id: str) -> int:
        events = self.list_unlinked_for_session(dealership_id, session_id)
        for event in events:
            event.lead_public_id = lead_public_id
            self.db.add(event)
        if events:
            self.db.commit()
        return len(events)

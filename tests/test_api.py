...
Path("test_[your_brand]_auto_sales.db").unlink(missing_ok=True)
os.environ.setdefault("DATABASE_URL", "sqlite:///./test_[your_brand]_auto_sales.db")
...
def test_anonymous_events_replay_into_lead_after_create():
    with make_client() as client:
        session_id = "anonymous-before-create-001"
        first_event = client.post(
            ...
            json={
                ...
                "sessionId": session_id,
                "eventType": "INVENTORY_DWELL_TIME",
                "data": {"durationSeconds": 300, "VehicleName": "Kia Seltos GTX"},
            },
        )
        assert first_event.status_code == 201
        assert first_event.json()["status"] == "accepted"
        ...
        create_payload = {
            ...
            "data": {
                ...
                "SessionId": session_id,
                "VehicleId": 1002,
                "VehicleName": "Kia Seltos GTX",
                "Message": "Can I book a test drive this week?",
            },
        }
        created = client.post("/api/lead/event", headers=headers, json=create_payload)
        assert created.status_code == 201
        body = created.json()
        assert body["lead"]["intent_signals"]["vehicle_page_time_seconds"] >= 300
        assert body["lead"]["intent_signals"]["test_drive_interest"] is True
        assert "Replayed" in body["detail"]

        timeline = client.get(f"/api/v1/leads/{body['leadPublicId']}/timeline", headers=headers)
        assert timeline.status_code == 200
        assert any(item["type"] == "INVENTORY_DWELL_TIME" for item in timeline.json())

        breakdown = client.get(f"/api/v1/leads/{body['leadPublicId']}/score-breakdown", headers=headers)
        assert breakdown.status_code == 200
        assert breakdown.json()["components"]["vehicle_dwell_points"] >= 10

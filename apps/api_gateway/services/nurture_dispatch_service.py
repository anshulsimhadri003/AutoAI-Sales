...
class NurtureDispatchService:
        ...
        for sequence in sequences:
            ...
            <html>
              <body style="font-family: Arial, sans-serif; color: #1f2937;">
                ...
                <p>{body}</p>
                <p>Thank you,<br/>[YourBrand] Auto Sales</p>
              </body>
            </html>
            """
    ...
    def _build_subject(sequence, template, lead) -> str:
        ...
        if template.template_type == "reminder":
            return f"[YourBrand] Auto: Reminder for {lead_name or lead.public_id}"
        if template.intent_tag == "test_drive":
            return f"[YourBrand] Auto: Schedule your test drive"
        if template.intent_tag == "finance":
            return f"[YourBrand] Auto: Finance options for your vehicle"
        if template.intent_tag == "availability":
            return f"[YourBrand] Auto: Availability update"
        return f"[YourBrand] Auto: Follow-up for {lead_name or lead.public_id}"
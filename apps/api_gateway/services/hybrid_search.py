...
def _load_faiss():
    """Import FAISS lazily so local tests/deploys do not hang or fail on unsupported platforms."""
...
class HybridSearchIndex:
    ...
    def _build_faiss_index(self, embeddings: np.ndarray):
        if embeddings.size == 0 or not self.settings.semantic_faiss_enabled:
            return None
